# from: https://github.com/serengil/deepface/blob/master/deepface/models/spoofing/FasNet.py
...
class Fasnet:
    ...
    def analyze(self, img: np.ndarray, facial_area: Union[list, tuple]):
        ...
        with torch.no_grad():
            ...
            first_result = F.softmax(first_result, dim=-1).cpu().numpy()

            second_result = self.second_model.forward(second_img)
            second_result = F.softmax(second_result, dim=-1).cpu().numpy()
...
if __name__ == "__main__":
    ...
    model_path = "detection\models\scrfd_2.5g.onnx"
    if not os.path.isfile(model_path):
        raise FileNotFoundError(f"❌ File not found: {model_path}")
    

    image_path = r"C:\Users\HP\Desktop\dev\easy-retina\detection\anti_spoof\test_images\WIN_20251008_19_56_46_Pro.jpg"
    image = cv2.imread(image_path)

    detector = SCRFD(model_path=model_path)
    boxes_list, _ = detector.detect(image)

    x, y, w, h, _ = boxes_list[0]

    antispoof_model = Fasnet()
    is_real, antispoof_score = antispoof_model.analyze(img=image, facial_area=(x, y, w, h))
    print(f"is_real: {is_real} | score: {antispoof_score}")
